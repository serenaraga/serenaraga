...
const sizeConfig = {
  sm: { emoji: "size-8 sm:size-9", label: "text-[10px]" },
  md: { emoji: "size-11 sm:size-13", label: "text-xs" },
  lg: { emoji: "size-16 sm:size-20", label: "text-sm" },
} as const;
...
export function Rating02({
  ...
  return (
    <div
      ...
      className={cn(
        "inline-flex items-center justify-between sm:justify-start gap-1.5 sm:gap-4 w-full sm:w-auto",
      ...
      {RATINGS.slice(0, max).map((rating, index) => {
        ...
        return (
          <motion.button
            ...
            disabled={disabled}
            className="group flex flex-1 sm:flex-initial flex-col items-center gap-1.5 p-1.5 rounded-xl outline-none transition-colors hover:bg-muted/40 focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 cursor-pointer"
          ...
          >
            <motion.img
              ...
              className={cn(
                ...
                isActive ? "drop-shadow-none" : "grayscale opacity-50 contrast-75",
              )}
              animate={imgAnimate}
            ...
            <span
              className={cn(
                ...
                "transition-colors duration-300 text-center leading-tight font-medium",
                isActive
